...
export default function Members() {
  return (
    <section id="members" className="px-6 md:px-12 py-20 md:py-32 relative"
      style={{ background: '#1a1a1a', borderTop: '1px solid rgba(192,57,43,0.2)' }}>
      <SectionDivider label="/ / 002 — Склад" />

      <div className="grid grid-cols-2 md:grid-cols-4 reveal"
        style={{ gap: '1px', background: 'rgba(192,57,43,0.2)', border: '1px solid rgba(192,57,43,0.2)' }}>
        {MEMBERS.map((m) => (
          <div key={m.num}
            className="group relative overflow-hidden p-6 md:p-10 transition-colors duration-300"
          ...
          >
            ...
            <div className="font-vt323 text-4xl md:text-5xl leading-none mb-3 md:mb-4"
              style={{ color: 'rgba(192,57,43,0.15)' }}>
              {m.num}
            </div>
            <div className="font-bebas text-xl md:text-2xl tracking-wide mb-1">{m.name}</div>
            ...
            <a href={m.ig} target="_blank" rel="noopener noreferrer"
              className="text-[0.6rem] tracking-wide no-underline transition-colors duration-200 hidden md:block"
            ...
            <a href={m.ig} target="_blank" rel="noopener noreferrer"
              className="text-[0.6rem] tracking-wide no-underline text-red md:hidden">
              IG ↗
            </a>
          </div>
        ))}
